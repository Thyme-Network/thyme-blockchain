...
from thyme.cmds.init_funcs import thyme_full_version_str
from thyme.protocols.protocol_message_types import ProtocolMessageTypes
from thyme.protocols.shared_protocol import Capability, Handshake
...
from thyme.util.ints import uint8, uint16

# Each message is prepended with LENGTH_BYTES bytes specifying the length
from thyme.util.network import class_for_type, is_localhost

# Max size 2^(8*4) which is around 4GiB
LENGTH_BYTES: int = 4


class WSThymeConnection:
    """
    ...
    async def perform_handshake(self, network_id: str, protocol_version: str, server_port: int, local_type: NodeType):
        if self.is_outbound:
            ...
            if ProtocolMessageTypes(inbound_handshake_msg.type) != ProtocolMessageTypes.handshake:
                raise ProtocolError(Err.INVALID_HANDSHAKE)
            if inbound_handshake.network_id != 'thyme-' + network_id:
