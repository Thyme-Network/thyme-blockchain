...
from thyme.plotting.plot_tools import find_duplicate_plot_IDs, get_plot_filenames, load_plots, parse_plot_info
from thyme.util.config import load_config
from thyme.util.hash import std_hash
from thyme.util.keychain import Keychain
from thyme.wallet.derive_keys import master_sk_to_farmer_sk, master_sk_to_local_sk
...
def check_plots(root_path, num, challenge_start, grep_string, list_duplicates, debug_show_memo):
    ...
    if len(no_key_filenames) > 0:
        log.warning(
            ...
            f"farm them, use 'thyme keys' to transfer keys. The pool public keys must be in the config.yaml"
        )
