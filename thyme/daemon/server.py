...
from thyme.cmds.init_funcs import thyme_init
from thyme.daemon.windows_signal import kill
from thyme.server.server import ssl_context_for_root, ssl_context_for_server
from thyme.ssl.create_ssl import get_mozzila_ca_crt
from thyme.util.thyme_logging import initialize_logging
from thyme.util.config import load_config
from thyme.util.json_util import dict_to_json_str
from thyme.util.path import mkdir
from thyme.util.service_groups import validate_service
from thyme.util.setproctitle import setproctitle
from thyme.util.ws_message import WsRpcMessage, create_payload, format_response
...
try:
    ...
    print("Error: Make sure to run . ./activate from the project folder before starting Thyme.")
    quit()
...
service_plotter = "thyme plots create"
...
if getattr(sys, "frozen", False):
    name_map = {
        "thyme": "thyme",
        "thyme_wallet": "start_wallet",
        "thyme_full_node": "start_full_node",
        "thyme_harvester": "start_harvester",
        "thyme_farmer": "start_farmer",
        "thyme_introducer": "start_introducer",
        "thyme_timelord": "start_timelord",
        "thyme_timelord_launcher": "timelord_launcher",
        "thyme_full_node_simulator": "start_simulator",
    }
...
class WebSocketServer:
    ...
    async def exit(self) -> Dict[str, Any]:
        ...
        log.info("thyme daemon exiting in 5 seconds")
...
def launch_plotter(root_path: Path, service_name: str, service_array: List[str], id: str):
    # we need to pass on the possibly altered THYME_ROOT
    os.environ["THYME_ROOT"] = str(root_path)
...
def launch_service(root_path: Path, service_command) -> Tuple[subprocess.Popen, Path]:
    ...
    log.debug(f"Launching service with THYME_ROOT: {os.environ['THYME_ROOT']}")
...
async def async_run_daemon(root_path: Path) -> int:
    thyme_init(root_path)
    config = load_config(root_path, "config.yaml")
    setproctitle("thyme_daemon")
