...
from thyme.protocols import wallet_protocol
from thyme.protocols.full_node_protocol import RequestProofOfWeight, RespondProofOfWeight
from thyme.protocols.protocol_message_types import ProtocolMessageTypes
from thyme.protocols.wallet_protocol import (
...
from thyme.util.errors import Err, ValidationError
from thyme.util.ints import uint32, uint128
from thyme.util.keychain import Keychain
from thyme.util.lru_cache import LRUCache
from thyme.util.merkle_set import MerkleSet, confirm_included_already_hashed, confirm_not_included_already_hashed
...
from thyme.wallet.settings.settings_objects import BackupInitialized
...
from thyme.wallet.wallet_blockchain import ReceiveBlockResult
from thyme.wallet.wallet_state_manager import WalletStateManager
...
class WalletNode:
    ...
    def get_key_for_fingerprint(self, fingerprint: Optional[int]):
        ...
        if len(private_keys) == 0:
            self.log.warning("No keys present. Create keys with the UI, or with the 'thyme keys' program.")
            return None
    ...
    async def on_connect(self, peer: WSThymeConnection):
        if self.wallet_state_manager is None or self.backup_initialized is False:
            return None
    ...
    async def complete_blocks(self, header_blocks: List[HeaderBlock], peer: WSThymeConnection):
    ...
    async def new_peak_wallet(self, peak: wallet_protocol.NewPeakWallet, peer: WSThymeConnection):
        if self.wallet_state_manager is None:
            return None
    ...
    async def _sync(self) -> None:
        ...
        peers: List[WSThymeConnection] = self.server.get_full_node_connections()
        if len(peers) == 0:
    ...
    async def get_additions(self, peer: WSThymeConnection, block_i, additions) -> Optional[List[Coin]]:
        if len(additions) > 0:
    ...
    async def get_removals(self, peer: WSThymeConnection, block_i, additions, removals) -> Optional[List[Coin]]:
        assert self.wallet_state_manager is not None
