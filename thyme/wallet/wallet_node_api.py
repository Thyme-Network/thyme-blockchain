from thyme.protocols import full_node_protocol, introducer_protocol, wallet_protocol
...
from thyme.util.api_decorators import api_request, peer_required, execute_task
from thyme.util.errors import Err
from thyme.wallet.wallet_node import WalletNode
...
class WalletNodeAPI:
    ...
    @peer_required
    @api_request
    async def respond_removals(self, response: wallet_protocol.RespondRemovals, peer: WSThymeConnection):
        pass

    async def reject_removals_request(self, response: wallet_protocol.RejectRemovalsRequest, peer: WSThymeConnection):
        """
    ...
    @peer_required
    @api_request
    async def respond_additions(self, response: wallet_protocol.RespondAdditions, peer: WSThymeConnection):
        pass
    ...
    @peer_required
    @api_request
    async def transaction_ack(self, ack: wallet_protocol.TransactionAck, peer: WSThymeConnection):
        """
    ...
    async def respond_peers_introducer(
        self, request: introducer_protocol.RespondPeersIntroducer, peer: WSThymeConnection
    ):
    ...
    @peer_required
    @api_request
    async def respond_peers(self, request: full_node_protocol.RespondPeers, peer: WSThymeConnection):
        if not self.wallet_node.has_full_node():
