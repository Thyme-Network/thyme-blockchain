...
from thyme.cmds.units import units
from thyme.rpc.wallet_rpc_client import WalletRpcClient
from thyme.server.start_wallet import SERVICE_NAME
...
def print_transaction(tx: TransactionRecord, verbose: bool, name) -> None:
    if verbose:
        ...
        thyme_amount = Decimal(int(tx.amount)) / units["thyme"]
        to_address = encode_puzzle_hash(tx.to_puzzle_hash, name)
        print(f"Transaction {tx.name}")
        print(f"Status: {'Confirmed' if tx.confirmed else ('In mempool' if tx.is_in_mempool() else 'Pending')}")
        print(f"Amount: {thyme_amount} {name}")
...
async def send(args: dict, wallet_client: WalletRpcClient, fingerprint: int) -> None:
    ...
    final_amount = uint64(int(amount * units["thyme"]))
    final_fee = uint64(int(fee * units["thyme"]))
    ...
    while time.time() - start < 10:
        ...
        if len(tx.sent_to) > 0:
            ...
            print(f"Do thyme wallet get_transaction -f {fingerprint} -tx 0x{tx_id} to get status")
            return None
...
async def print_balances(args: dict, wallet_client: WalletRpcClient, fingerprint: int) -> None:
    ...
    for summary in summaries_response:
        ...
        if typ != "STANDARD_WALLET":
            ...
            print(
                f"   -Total Balance: {balances['confirmed_wallet_balance']/units['thyme']} {address_prefix} "
                f"({balances['confirmed_wallet_balance']} mojo)"
            )
            print(
                f"   -Pending Total Balance: {balances['unconfirmed_wallet_balance']/units['thyme']} {address_prefix} "
                f"({balances['unconfirmed_wallet_balance']} mojo)"
            )
            print(
                f"   -Spendable: {balances['spendable_balance']/units['thyme']} {address_prefix} "
...
async def get_wallet(wallet_client: WalletRpcClient, fingerprint: int = None) -> Optional[Tuple[WalletRpcClient, int]]:
    ...
    if len(fingerprints) == 0:
        print("No keys loaded. Run 'thyme keys generate' or import a key")
        return None
    ...
    if log_in_response["success"] is False:
        if log_in_response["error"] == "not_initialized":
            ...
            if "backup_path" in log_in_response:
                ...
                print(f"Backup file from backup.chianetwork.org downloaded and written to: {path}")
