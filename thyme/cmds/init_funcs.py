...
from thyme import __version__
from thyme.consensus.coinbase import create_puzzlehash_for_pk
from thyme.ssl.create_ssl import generate_ca_signed_cert, get_thyme_ca_crt_key, make_ca_cert
from thyme.util.bech32m import encode_puzzle_hash
from thyme.util.config import (
    create_default_thyme_config,
    initial_config_file,
    load_config,
    save_config,
...
from thyme.util.keychain import Keychain
from thyme.util.path import mkdir
from thyme.wallet.derive_keys import master_sk_to_pool_sk, master_sk_to_wallet_sk
...
def check_keys(new_root: Path) -> None:
    ...
    if len(all_sks) == 0:
        print("No keys are present in the keychain. Generate them with 'thyme keys generate'")
        return None

    config: Dict = load_config(new_root, "config.yaml")
    pool_child_pubkeys = [master_sk_to_pool_sk(sk).get_g1() for sk, _ in all_sks]
    all_targets = []
    stop_searching_for_farmer = "xth_target_address" not in config["farmer"]
    stop_searching_for_pool = "xth_target_address" not in config["pool"]
    ...
    for i in range(number_of_ph_to_search):
        ...
        for sk, _ in all_sks:
            ...
            if all_targets[-1] == config["farmer"].get("xth_target_address"):
                stop_searching_for_farmer = True
            if all_targets[-1] == config["pool"].get("xth_target_address"):
                stop_searching_for_pool = True

    # Set the destinations
    if "xth_target_address" not in config["farmer"]:
        print(f"Setting the xth destination address for coinbase fees reward to {all_targets[0]}")
        config["farmer"]["xth_target_address"] = all_targets[0]
    elif config["farmer"]["xth_target_address"] not in all_targets:
        print(
            f"WARNING: using a farmer address which we don't have the private"
            f" keys for. We searched the first {number_of_ph_to_search} addresses. Consider overriding "
            f"{config['farmer']['xth_target_address']} with {all_targets[0]}"
        )

    if "pool" not in config:
        config["pool"] = {}
    if "xth_target_address" not in config["pool"]:
        print(f"Setting the xth destination address for coinbase reward to {all_targets[0]}")
        config["pool"]["xth_target_address"] = all_targets[0]
    elif config["pool"]["xth_target_address"] not in all_targets:
        print(
            f"WARNING: using a pool address which we don't have the private"
            f" keys for. We searched the first {number_of_ph_to_search} addresses. Consider overriding "
            f"{config['pool']['xth_target_address']} with {all_targets[0]}"
        )
...
def create_all_ssl(root: Path):
    ...
    thyme_ca_crt, thyme_ca_key = get_thyme_ca_crt_key()
    thyme_ca_crt_path = ca_dir / "thyme_ca.crt"
    thyme_ca_key_path = ca_dir / "thyme_ca.key"
    thyme_ca_crt_path.write_bytes(thyme_ca_crt)
    thyme_ca_key_path.write_bytes(thyme_ca_key)
    ...
    thyme_ca_crt, thyme_ca_key = get_thyme_ca_crt_key()
    generate_ssl_for_nodes(ssl_dir, thyme_ca_crt, thyme_ca_key, False, overwrite=False)
...
def init(create_certs: Optional[Path], root_path: Path):
    if create_certs is not None:
        if root_path.exists():
            ...
            print("** Please run `thyme init` to migrate or create new config files **")
    else:
        return thyme_init(root_path)


def thyme_version_number() -> Tuple[str, str, str, str]:
...
def thyme_minor_release_number():
    res = int(thyme_version_number()[2])
    print(f"Install release number: {res}")
    return res


def thyme_full_version_str() -> str:
    major, minor, patch, dev = thyme_version_number()
    return f"{major}.{minor}.{patch}{dev}"


def thyme_init(root_path: Path):
    if os.environ.get("THYME_ROOT", None) is not None:
        print(
            f"warning, your THYME_ROOT is set to {os.environ['THYME_ROOT']}. "
            f"Please unset the environment variable and run thyme init again\n"
            f"or manually migrate config.yaml"
        )

    print(f"Thyme directory {root_path}")
    if root_path.is_dir() and Path(root_path / "config" / "config.yaml").exists():
        # This is reached if THYME_ROOT is set, or if user has run thyme init twice
        # before a new update.
        check_keys(root_path)
        print(f"{root_path} already exists, no migration action taken")
        return -1

    create_default_thyme_config(root_path)
    create_all_ssl(root_path)
    check_keys(root_path)
    print("")
    print("To see your keys, run 'thyme keys show --show-mnemonic-seed'")
