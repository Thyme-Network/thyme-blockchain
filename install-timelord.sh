...
THYMEVDF_VERSION=$(python -c 'from setup import dependencies; t = [_ for _ in dependencies if _.startswith("chiavdf")][0]; print(t)')
...
if [ -e "$THE_PATH" ]; then
	...
	if [ -e venv/bin/python ] && test $UBUNTU_DEBIAN; then
		...
		echo venv/bin/python -m pip install --force --no-binary chiavdf "$THYMEVDF_VERSION"
		venv/bin/python -m pip install --force --no-binary chiavdf "$THYMEVDF_VERSION"
