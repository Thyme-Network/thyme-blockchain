...
from thyme.daemon.server import WebSocketServer, create_server_for_daemon, daemon_launch_lock_path, singleton
from thyme.full_node.full_node_api import FullNodeAPI
from thyme.server.start_farmer import service_kwargs_for_farmer
from thyme.server.start_full_node import service_kwargs_for_full_node
from thyme.server.start_harvester import service_kwargs_for_harvester
from thyme.server.start_introducer import service_kwargs_for_introducer
from thyme.server.start_service import Service
from thyme.server.start_timelord import service_kwargs_for_timelord
from thyme.server.start_wallet import service_kwargs_for_wallet
from thyme.simulator.start_simulator import service_kwargs_for_full_node_simulator
from thyme.timelord.timelord_launcher import kill_processes, spawn_process
from thyme.types.peer_info import PeerInfo
from thyme.util.bech32m import encode_puzzle_hash
from thyme.util.block_tools import BlockTools, test_constants
from thyme.util.hash import std_hash
from thyme.util.ints import uint16, uint32
...
async def setup_farmer(
    ...
    config["xth_target_address"] = encode_puzzle_hash(b_tools.farmer_ph, "xth")
    config["pool_public_keys"] = [bytes(pk).hex() for pk in b_tools.pool_pubkeys]
    config["port"] = port
    config_pool["xth_target_address"] = encode_puzzle_hash(b_tools.pool_ph, "xth")
