...
async def disconnect_all_and_reconnect(server: ThymeServer, reconnect_to: ThymeServer) -> bool:
...
async def add_dummy_connection(server: ThymeServer, dummy_port: int) -> Tuple[asyncio.Queue, bytes32]:
    ...
    generate_ca_signed_cert(
        server.thyme_ca_crt_path.read_bytes(), server.thyme_ca_key_path.read_bytes(), dummy_crt_path, dummy_key_path
    )
    ssl_context = ssl_context_for_client(
        server.thyme_ca_crt_path, server.thyme_ca_key_path, dummy_crt_path, dummy_key_path
    )
...
async def connect_and_get_peer(server_1: ThymeServer, server_2: ThymeServer) -> WSThymeConnection:
    """
